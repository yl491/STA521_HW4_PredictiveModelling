...
for (i in 1:nsim)
{
  ...
  y.rep[i,]=rnorm(n,mean=mu,sd=sim.fit1@sigma[i])
...
rmse.fit1<-apply(y.rep,1,function(y) rmse(y,Col.lm.predict))
rmse.observations<-rmse(College.train$Apps,Col.lm.predict)

df.fit1<-data.frame(rmse.fit1=rmse.fit1)
...
mean(rmse.observations>rmse.fit1)
...
Col.lm2<-lm(log(Apps)~log(F.Undergrad)+I(perc.alumni^2)+Grad.Rate+log(Expend)+log(Room.Board)+Private:Elite,data=College.train)
summary(Col.lm2)
plot(Col.lm2)
...
nsim=1000
n=nrow(College.train.rmca)
X=model.matrix(Col.lm2)
sim.fit2=sim(Col.lm2,nsim)
y.rep=array(NA,c(nsim,n))
for (i in 1:nsim)
{
  mu=exp(X %*% sim.fit2@coef[i,])
  y.rep[i,]=rnorm(n,mean=mu,sd=sim.fit2@sigma[i])
  
}
Col.lm.predict2<-predict(Col.lm2,data=College.train)

rmse.fit2<-apply(y.rep,1,function(y) rmse(y,Col.lm.predict2))
rmse.observations<-rmse(College.train$Apps,Col.lm.predict2)

df.fit1<-data.frame(rmse.fit2=rmse.fit2)
ggplot(df.fit21,aes(x=rmse.fit2))+geom_histogram()+geom_vline(xintercept=rmse.observations,col=2)
