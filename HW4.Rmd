...
col.train.glm <- glm(Apps ~log(F.Undergrad)+I(perc.alumni^2)+Grad.Rate+log(Expend)+log(Room.Board)+Private:Elite, data=College.train.rmca,family=poisson(link="log"))

poi.yhat.train=predict(col.glm,type="response")
poi.yhat.test=predict(col.glm,newdata=College.test.rmca,type="response")
